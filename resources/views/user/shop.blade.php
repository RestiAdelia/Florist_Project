...
@section('content')
    <div class="pt-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @foreach ($products as $product)
                <div
                    ...
                    <div class="p-3 sm:p-4 flex flex-col flex-grow">
                        ...
                        {{-- Nama Produk (link ke detail)  route('product.show', $product->id) --}}
                        ...
                        {{-- Jenis
                        <p class="text-xs text-gray-500 mb-2 capitalize">{{ $product->jenis }}</p> --}}

                        {{-- Rating & Jumlah Review --}}
                        <div class="flex items-center text-sm text-gray-600 mb-3">
                            <div class="flex text-yellow-400">
                                {{-- Placeholder untuk rating, Anda perlu ganti dengan logika dinamis --}}
                                @php
                                    $averageRating = $product->average_rating ?? 4.5; // Contoh data rating
                                    $numberOfReviews = $product->reviews_count ?? 120; // Contoh jumlah review
                                @endphp
                                @for ($i = 1; $i <= 5; $i++)
                            ...
                            {{-- Link ke halaman review produk (jika ada)route('product.show', ['id' => $product->id, '#reviews']) --}}
                            <a href="{{ '#' }}" class="ml-2 text-gray-500 hover:text-teal-600">
                                ({{ $numberOfReviews }} review)
                            </a>
                        </div>

                        {{-- Harga di kiri, Tombol Tambah ke Keranjang di kanan --}}
        ...
        {{-- Jika $products di-paginate di controller, Anda bisa mengaktifkan ini --}}
